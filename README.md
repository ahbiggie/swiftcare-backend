...
| Area                                    | Owner                    |
| --------------------------------------- | ------------------------ |
| Auth & accounts · Queue + state machine | Shaibu (Lane 1)          |
...
- **Queue transitions guard**: `assertCanTransition()` in [transitions.js](src/services/queue/transitions.js), the full rulebook including cancellation and required notes, verified in [queue-transitions.test.js](tests/queue-transitions.test.js).
...
- **Auth, complete**: `POST /auth/clinic/signup`, `/login`, `/invite`, `/accept-invite`, plus `GET /auth/me`, `GET /users`, and `GET /staff/doctors`. Own JWT util, own tests, a proven concurrency guard on invite acceptance. Sitting in an open PR awaiting review; every route in contract section 1 is covered once it merges.
