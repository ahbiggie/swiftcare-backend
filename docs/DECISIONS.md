...
Wires `GET /queue` and `POST /queue/:queueId/status` to the real model, which turns the rulebook (D5, D8) into a callable endpoint. Four decisions came out of it.
...
**`changedBy` is a soft reference with no FK** _(fixed a real defect, caught by the route tests)_. The first cut FK-constrained it to `staff`, which passed every test until the admin-override case: **admin is the clinic account**, so its token subject is a *clinic* id, and writing that into a staff-constrained column raised `23503` — a documented feature returning `500`. One FK cannot point at two tables. For an append-only audit log the id matters more than referential integrity, so the constraint is dropped and the column records whichever kind of account acted. Correspondingly `queue_entries.lastUpdatedBy` — which *is* FK-constrained to `staff`, from an already-merged migration — is written only for staff actors and left `null` for admin; the event row keeps the true attribution for everyone. Worth noting for whoever adds the next actor-stamped column: any "who did this" field has the same two-kinds-of-account problem.
